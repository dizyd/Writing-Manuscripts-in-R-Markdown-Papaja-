...
```{r, eval = FALSE} 
fit <- lm(body_mass_g ~ bill_length_mm + flipper_length_mm, data = na.omit(palmerpenguins::penguins))
...
apa_table(apa_print_fit$table,caption = "This is a regression table",
          note="$M_I$ $\\frac{3}{4}$ ", escape = FALSE) #<<
```
...
```{r, eval = FALSE} 
fit2 <- lm(body_mass_g ~ bill_length_mm + flipper_length_mm + sex, data = na.omit(palmerpenguins::penguins))


mod_comparison <- apa_print(list(Baseline = fit,
                                 Sex      = fit2),
                            boot_samples=0)

apa_table(mod_comparison$table,
...
.pull-right[
<img src="images/example_apatable_print3.PNG", width="100%">
]
...
apa_table(mod_comparison$table[c(1:5,10:12),], #<<
          caption = "This is a regression table",
          note    = "Cool Table")
```
...
.pull-right[
<img src="images/example_apatable_print4.PNG", width="100%">
]
...
Sometimes there are also additional `R` packages for specific table types compatible with `apa_table` (e.g., `corx` or `apaTables`)
...
<img src="images/table6.PNG", width="100%">
...
- If not already done, create a new papaja-.Rmd document

- load the [`penguins`](https://allisonhorst.github.io/palmerpenguins/) data  (`library(palmerpenguins)`)


-  make a publication ready table with `kable()` showing the median, maximum, and minimum of `body_mass_g`  for each `species`, as well as the correlation with `bill_length_mm`

- calculate a ANOVA comparing `body_mass_g` with `sex` and `island` as between factors (`aov(body_mass_g ~ sex + island, data = penguins)` ))

- Report your results for one of the factors (e.g., `sex` or `island`) in the text using `apa_print()`
...
`papaja` also provides a custom `ggplot` theme (`theme_apa`)
...
apa_beeplot(
   data    = penguins %>% mutate(ID = 1:nrow(.)),
   id      = "ID",
   dv      = "body_mass_g",
   factors = c("year","island"),
   ylab    = "Body Mass (g)",
   xlab    = "Year")
...
You can also adjust the **figure size**, **dpi**, and add a **figure caption** via the code chunk options. For instance, adding `fig.cap="These are some penguins"` to the chunk leads to : 
...
- Plot the ANOVA you calculated before using one of the built-in plot functions (`apa_beeplot(), apa_lineplot(), apa_barplot(), apa_factorial_plot()`), or build your own plot ! 

- Play around with the `fig.cap`, `fig.width`, and `fig.height` arguments in the chunk options of your plot
...
- Add a name to the code chunks of your tables and plots

- Refer to both in text using `\@ref(...)`

- Add a new citation to the `mybib.bib` file (e.g., Birnbaum, M. H. (1999), https://doi.org/10.1037/1082-989X.4.3.243)
